...
function assembleReport(v) {
  ...
  // ─── OPENING FRAME ───
  let opening_frame = isZeroAmp ? SEC01_ZERO_AMP : (SEC01[state] || "");
  opening_frame = opening_frame.replace(/\{\{leader_name\}\}/g, name);
  opening_frame = opening_frame.replace(/\{\{snapshot_date\}\}/g, date);
  opening_frame = wrap(opening_frame);
  ...
  return {
    ...
    territories_preamble: territories_preamble,
