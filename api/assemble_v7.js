// ════════════════════════════════════════════════════════════════
// LEADER COHERENCE SNAPSHOT \u2014 CONTENT ASSEMBLY ENGINE v7
...
// v7 changes:
//   - opening_frame: field retained in output but returns empty string (removed from Writer p4)
//   - defining_flag_note: duplicate removed from Writer p4 upper block (kept in Overall Diagnosed State only)
//   - territories_preamble replaced by territories_opening (generateTerritoriesOpening())
//   - feltPct extracted from v.felt_gap_pct for delta calculation
//   - buildSec05 / sec05_html retained in output (dead field — Writer template removed it in v2+)
// ════════════════════════════════════════════════════════════════
...
function generateTerritoriesOpening(state, gap, diagPct, feltPct, bState, rState, cState, isZeroAmp, definingFlag, breadthOverride) {
  ...
  if (state === "COHERENT") {
    ...
    const STATE_SIGNATURES = {
      ...
      "Fractured": "significant disconnection from operating self"
    };
